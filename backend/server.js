...
const adminLimiter = rateLimit({
    windowMs: 1 * 60 * 1000,
    max: 500,
    message: 'Admin terlalu aktif, mohon jeda sebentar.'
});
...
app.get('/api/admin/voters', verifyAdminToken, async (req, res, next) => {
    try {
        ...
        const [voters] = await db.execute(
            'SELECT nim, nama, status_pilih, token_diambil, waktu_pilih, token FROM pemilih ORDER BY nim ASC'
        );
        ...
        logger.error('Get all voters error', { error: error.message });
        next(error);
    }
});
...
/**
 * POST /api/admin/voters/activate-all
 * Activate all tokens for inactive voters
 * Headers: Authorization: Bearer <adminToken>
 */
app.post('/api/admin/voters/activate-all', verifyAdminToken, async (req, res, next) => {
    try {
        const [result] = await db.execute(
            `UPDATE pemilih SET token_diambil = 1 WHERE token_diambil = 0`
        );

        logger.info('Activate all tokens', { 
            count: result.affectedRows,
            adminId: req.admin.adminId 
        });
        ...
        logger.error('Activate all error', { error: error.message });
        next(error);
    }
});
